...
class AM_Controller extends CI_Controller {
    ...
    function index() {
        ...
        $data['total_users'] = $this->am->getTotalUsers();
        $data['submitted_evaluation'] = $this->am->getTotalSubmittedEvaluation();
        $data['peding_evaluation'] = $this->am->getTotalPedingEvaluation();
        $data['assign_employee'] = $this->am->getTotalAssignEmployee();

        $data['total_admin'] = $this->am->getTotalAdmin();
        $data['total_supervisor'] = $this->am->getTotalSupervisor();
        $data['total_employee'] = $this->am->getTotalEmployee();
...
//------------------------------------------------------------------------------------------
   ...
   // Get Employee List
    ...
    public function getEmployeeRecords(){
        ...
        foreach ($resultList as $key => $value) {
            ...
            $button = '<div class="dropdown">
                ...
                <a class="dropdown-item" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#trashEmployee'.$value['main_employee_id'].'">
                    <i class="bx bx-trash-alt me-1"></i> Delete
                </a>
                </div>
            </div>
            <div class="modal fade" id="trashEmployee'.$value['main_employee_id'].'" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            <p>Are you sure you want to delete this employee? Currenty moving on the trash.</p>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button>
                            <a href="'.base_url('admin/AM_Controller/trashEmployeeRecord/'.$value['main_employee_id']).'" class="btn btn-danger"><i class="bx bx-trash-alt me-1"></i> Trash</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    public function trashEmployeeRecord($id){
        $data = [
            'employee_status' => 0
        ];
        $result = $this->am->updateEmpDetails($id, $data);
        if($result){
            $this->session->set_flashdata('success', 'Employee Deleted Successfully!!!');
            redirect('admin/AM_Controller/viewEmployeeList');
        }else{
            $this->session->set_flashdata('error', 'Please Try Again!');
            redirect('admin/AM_Controller/viewEmployeeList');
        }
    ...
    public function viewEmployeeProfile($id){
        ...
        $data['employee_leaves'] = $this->am->getEmployeeLeaves($id);
        $data['employee_salary'] = $this->am->getEmployeeSalary($id);
        $data['employee_personality'] = $this->am->getEmployeePersonality($id);
        $data['personality_rank'] = $this->am->getEmployeePersonalityRank($id);

        // echo '<pre>'; print_r($id); echo '</pre>';exit;

        $data['software_expertise'] = $this->am->getSoftwareExpertise($id);
...
//------------------------------------------------------------------------------------------
    ...
    function addSectionInEvaluationForm(){
        ...
        $data['section_instruction'] = $this->input->post('section_instruction');
                    ...
                    <div class="modal-content">
                        ...
                        <form action="'.base_url('admin/AM_Controller/updateSelect').'" method="POST">
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label class="form-label" for="basic-default-fullname">Section Instructions <span class="isrequired">*</span></label>
                                    <textarea class="form-control" name="section_instruction" id="" placeholder="Instructions">'.$value['section_instruction'].'</textarea>
                                </div>
...
//------------------------------------------------------------------------------------------
        ...
        public function getAssignEmployeeRecords($id){
            ...
            foreach ($resultList as $value) {
                $button = '<div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item text-danger" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#removeAssignModal'.$value['main_employee_id'].'">
                                    <i class="bx bx-trash me-1"></i> Remove Assign
                                </a>
                            </div>
                        </div>
                        
                        <!-- Delete Modal -->
                        <div class="modal fade" id="removeAssignModal'.$value['main_employee_id'].'" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Remove the assign access</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                                        <form action="'.base_url('admin/AM_Controller/removeAssignAccess').'" method="POST">
                                            <input type="hidden" name="assign_id" value="'.$value['assign_id'].'">
                                            <input type="hidden" name="main_employee_id" value="'.$value['main_employee_id'].'">
                                            <button type="submit" class="btn btn-danger">Delete</button>
                                        </form>
                                    </div>
                                </div>
        ...
        public function removeAssignAccess(){
            $assign_id = $this->input->post('assign_id');
            $main_employee_id = $this->input->post('main_employee_id');

            $res = $this->am->removeAssignAccess($assign_id);
            if($res){
                $this->session->set_flashdata('success', 'Assign Access Removed Successfully!!!');
                redirect('admin/AM_Controller/showEmployeeAssignEmployee/'.$main_employee_id);
            }   else{
                $this->session->set_flashdata('error', 'Failed to remove assign access. Please try again.');
                redirect('admin/AM_Controller/showEmployeeAssignEmployee/'.$main_employee_id);
            }
        }
...
// Admin can employee evaluation
    ...
    public function viewEmployeePerformanceList(){
        ...
        $columns = [
            ...
            ['data' => 4, 'title' => 'Level', 'filterType' => 'select'],
            ['data' => 5, 'title' => 'Spectrum', 'filterType' => 'select'],
            ['data' => 6, 'title' => 'Evaluation Status', 'filterType' => 'select'],
            ['data' => 7, 'title' => 'Actions', 'orderable' => false, 'filterable' => false]
        ];
        $data['columns'] = $columns;
                        ...
                        <div class="modal fade" id="trashEmployeeEvaluation'.$value['employee_evaluation_id'].'" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        '.($value['evaluation_status'] > 1 ? '<p>Are you sure you want to delete this evaluation?</p>' : '<p>This evaluation is not locked!!</p>').'
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button>
                                        '.($value['evaluation_status'] == 1 ? '<button type="button" class="btn btn-secondary"><i class="bx bx-trash-alt me-1" disabled></i> Trash</button>' : '<a href="'.base_url('admin/AM_Controller/trashEmployeeEvaluationRecord/'.$value['employee_evaluation_id']).'" class="btn btn-danger"><i class="bx bx-trash-alt me-1"></i> Trash</a>').'
                                    </div>
                                    </div>
                                </div>
                            </div>
    ...
    public function trashEmployeeEvaluationRecord($evaluation_id){
        ...
        $result = $this->am->trashEmployeeEvaluationRecordDB($evaluation_id, $data);
    ...
    // software expert
    public function addSoftwareExpertise(){
        ...
        $data['software_name'] = $this->input->post('software_name');
        $data['software_value'] = $this->input->post('software_value');

        if((!empty($_FILES['software_image']['name']))){
            $check = uploadimgfile("software_image",$folder="upload",$prefix="proimg_");
            $link  = $check['data']['name'];
            $data['software_image'] = $link;
        }

        $this->load->library('upload', $config);
        $this->upload->do_upload();

        $res = $this->am->addSoftwareExpertiseDB($data);
        if($res){
            $this->session->set_flashdata("success", "Software Expertise Added Successfully");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
            return;
        }else{
            $this->session->set_flashdata("error", "Failed to Add Software Expertise");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
            return;
        }
    }

    public function deleteSoftwareExpertise($id){
        $software_expertise_id = $this->input->post('software_expertise_id');
        $employee_id = $this->input->post('employee_id');
        $res = $this->am->deleteSoftwareExpertiseDB($software_expertise_id);
        if($res){
            $this->session->set_flashdata("success", "Software Expertise Deleted Successfully");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }else{
            $this->session->set_flashdata("error", "Failed to Delete Software Expertise");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }
    }
    ...
    public function addEmployeeLeaves(){
        ...
        // Prepare employee data
        $data['employee_id'] = $this->input->post('employee_id');
        $data['leave_url'] = $this->input->post('leave_url');
        $data['file_name'] = $this->input->post('file_name');
        $data['date_created'] = date('Y-m-d');

        $res = $this->am->addEmployeeLeavesDB($data);
        if($res){
            $this->session->set_flashdata("success", "Employee Leave Added Successfully");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
            return;
        }else{
            $this->session->set_flashdata("error", "Failed to Add Employee Leave");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
            return;
        }
    }

    public function deleteEmployeeLeaveRecord($id){
        $employee_id = $this->input->post('employee_id');
        $res = $this->am->deleteEmployeeLeaveRecordDB($employee_id);
        if($res){
            $this->session->set_flashdata("success", "Employee Leave Deleted Successfully");    
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }else{
            $this->session->set_flashdata("error", "Failed to Delete Employee Leave");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }
    }

    public function addPersonalityDetails(){
        // Prepare employee data
        $data['employee_id'] = $this->input->post('employee_id');
        $data['personality_type'] = $this->input->post('personality_type');
        $data['personality_role'] = $this->input->post('personality_role');
        $data['personality_code'] = $this->input->post('personality_code');
        $data['personality_strategy'] = $this->input->post('personality_strategy');

        $res = $this->am->addPersonalityDetailsDB($data);
        if($res){
            $this->session->set_flashdata("success", "Personality Details Added Successfully");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
            return;
        }else{
            $this->session->set_flashdata("error", "Failed to Add Personality Details");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
            return;
        }
    ...
    public function editPersonalityDetails(){
        ...
        $res = $this->am->editPersonalityDetailsDB($employee_id, $data);
        if($res){
            $this->session->set_flashdata("success", "Personality Details Updated Successfully");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }else{
            $this->session->set_flashdata("error", "Failed to Update Personality Details");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }
    }

    public function deletePersonalityDetails($id){
        $personality_id = $this->input->post('personality_id');
        $employee_id = $this->input->post('employee_id');
        $res = $this->am->deletePersonalityDetailsDB($personality_id, $employee_id);
        if($res){
            $this->session->set_flashdata("success", "Personality Details Deleted Successfully");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }else{
            $this->session->set_flashdata("error", "Failed to Delete Personality Details");
            redirect("admin/AM_Controller/viewEmployeeProfile/".$employee_id);
            return;
        }
    }
    ...
    public function addingPersonalityRanks(){

        $submit_condi = $this->input->post('submit_condi');

        if($submit_condi == 1){

            $data['employee_id'] = $this->input->post('employee_id');
            $data['min_value_mind'] = $this->input->post('min_value_mind');
            $data['max_value_mind'] = $this->input->post('max_value_mind');

            $data['min_value_energy'] = $this->input->post('min_value_energy');
            $data['max_value_energy'] = $this->input->post('max_value_energy');

            $data['min_value_nature'] = $this->input->post('min_value_nature');
            $data['max_value_nature'] = $this->input->post('max_value_nature');

            $data['min_value_tactics'] = $this->input->post('min_value_tactics');
            $data['max_value_tactics'] = $this->input->post('max_value_tactics');

            $data['min_value_identity'] = $this->input->post('min_value_identity');
            $data['max_value_identity'] = $this->input->post('max_value_identity');

            $res = $this->am->addingPersonalityRanksDB($data);
            if($res){
                $this->session->set_flashdata("success", "Personality Ranks Added Successfully");
                redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
                return;
            }else{
                $this->session->set_flashdata("error", "Failed to Add Personality Ranks");
                redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
                return;
            }
            ...
            $res = $this->am->editPersonalityRanksDB($employee_id, $data);
            if($res){
                $this->session->set_flashdata("success", "Personality Ranks Updated Successfully");
                redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
                return;
            }else{
                $this->session->set_flashdata("error", "Failed to Update Personality Ranks");
                redirect("admin/AM_Controller/viewEmployeeProfile/".$data['employee_id']);
            }
